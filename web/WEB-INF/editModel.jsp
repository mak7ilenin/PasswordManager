...
<div class="w-100 d-flex justify-content-center">
    <form action="editModel" method="POST" >
        <div class="card border-0 mb-3" style="width: 40em;">
            ...
            <label for="theModels" style="text-align: center;" class="form-label mt-4">Доступные модели</label>
            <select class="form-select"  id="theModels" name="theModels">
                <c:forEach var="model" items="${models}">
                    <option style="text-align: center" value="${model.id}" <c:if test="${model.id eq theModels}">${model.modelName}</c:if>>Model: ${model.modelName} // Size: ${model.modelSize} // Brand: ${model.modelFirm} // Price: ${model.price}</option>
                </c:forEach>
            </select>
